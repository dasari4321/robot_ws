...
class ScaledBaroNode(Node):
    def __init__(self):
        ...
        self.pressure_to_meters_scale = 0.0833 * 20.0 # Adjusted for your current observation
        
        self.p_ref = 101325.0
        self.z_offset = None

        self.sub_ref = self.create_subscription(FluidPressure, '/env/reference_pressure', self.ref_cb, 10)
        self.sub_robot = self.create_subscription(FluidPressure, '/air_pressure', self.robot_cb, 10)
        self.pub = self.create_publisher(PoseWithCovarianceStamped, '/baro_pose', 10)
    ...
    def robot_cb(self, msg):
        ...
        self.get_logger().info(f"Pressure Delta: {delta_p:.2f} Pa | Scaled Z: {final_z:.3f}m")
