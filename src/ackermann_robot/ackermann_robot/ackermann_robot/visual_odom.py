#!/usr/bin/env python3
"""Estimate odometry using visual ORB features."""
import math
...
class VisualOdom(Node):
    """Estimate visual odometry using ORB features and Essential Matrix."""

    def __init__(self):
        """Initialize the VisualOdom node."""
        ...
        # High threshold to start tracking (hysteresis)
        self.init_threshold = 50
        ...
        self.orb = cv2.ORB_create(
            nfeatures=self.min_features, scaleFactor=1.2, nlevels=8)
        ...
        self.create_subscription(
            CameraInfo, '/front_camera/camera_info', self.info_cb, 30)
    ...
    def info_cb(self, msg):
        """Callback for camera info to get intrinsic matrix."""
        if self.K is None:
            self.K = np.array(msg.k).reshape(3, 3)
            self.get_logger().info('Camera Info Received. Calibration set.')

    def odom_cb(self, msg):
        """Callback for wheel odometry to get speed for scaling."""
        vx = msg.twist.twist.linear.x
        vy = msg.twist.twist.linear.y
        self.current_speed = math.sqrt(vx**2 + vy**2)

    def image_cb(self, msg):
        """Callback for camera images to compute visual odometry."""
        if self.K is None:
        ...
        if self.prev_image is None:
            if len(kp) < self.init_threshold:
                self.get_logger().warn(
                    f'Waiting for better frame to init '
                    f'({len(kp)}/{self.init_threshold} features)...',
        ...
        if len(matches) < self.match_threshold:
            self.get_logger().warn(
                f'Insufficient matches ({len(matches)}). '
                'Tracking lost, resetting tracker.')
            self.prev_image = None
            return
        ...
        E, mask = cv2.findEssentialMat(
            pts2, pts1, K_curr, method=cv2.RANSAC, prob=0.999,
            threshold=2.0)
    ...
    def publish_odom(self, stamp):
        ...
        self.get_logger().info(
            f'Odom: x={float(self.cur_t[0]):.2f}, y={float(self.cur_t[1]):.2f}',
            throttle_duration_sec=1.0)


def main(args=None):
    """Run the VisualOdom node."""
